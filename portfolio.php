...
<html lang="en">
    ...
    <body>
        ...
        <main>
            ...
            $portfolioItems = json_decode($json, true);
            ...
            for ($i = 0; $i < $total; $i += $columnsPerRow) {
                echo '<div class="row">';
                for ($j = $i; $j < $i + $columnsPerRow && $j < $total; $j++) {
                    $item = $portfolioItems[$j];
                    echo '
                        <div class="col-25 portfolio text-white text-center" id="portfolio-' . $item['id'] . '">
                            <a href="' . htmlspecialchars($item['url']) . '" target="_blank">
                                <img src="' . htmlspecialchars($item['image']) . '" alt="' . htmlspecialchars($item['title']) . '" style="width:100%; border-radius: 8px; margin-bottom: 10px;">
